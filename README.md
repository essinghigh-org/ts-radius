...
| `sendAccounting` (`accountingStart` / `accountingInterim` / `accountingStop` / `accountingOn` / `accountingOff`) | ✅ Retries up to `retry.maxAttempts` on: `timeout`, `malformed_response`, `identifier_mismatch`, `authenticator_mismatch`, `unknown_code` | `validateResponseSource`, `responseLengthValidationPolicy`, `responseMessageAuthenticatorPolicy` | Uses `accountingPort` (default 1813). Retries to the same destination reuse one Accounting Identifier and source port (with deterministic Request Authenticator for identical attrs/secret). Helpers map to `Acct-Status-Type` values, including `Accounting-On` and `Accounting-Off`. If neither NAS-IP-Address (4) nor NAS-Identifier (32) is provided, a NAS-IP-Address of `127.0.0.1` is injected. `customAttributes` (`request.attributes`) reject User-Password (2), CHAP-Password (3), Reply-Message (18), State (24), Acct-Status-Type (40), and Acct-Session-Id (44). |
| `sendCoa` | ✅ Retries up to `retry.maxAttempts` on: `timeout`, `malformed_response`, `identifier_mismatch`, `authenticator_mismatch`, `unknown_code` | `validateResponseSource`, `responseLengthValidationPolicy`, `responseMessageAuthenticatorPolicy`, `dynamicAuthorizationRetryIdentityMode`, `dynamicAuthorizationEventTimestampWindowSeconds` | `coa_nak` is terminal (no retry). Unexpected dynamic response codes are silently discarded (the call keeps waiting for CoA-ACK/NAK until timeout). In `stable` mode, same-host retries reuse one identifier + source port while failover host retries get new values; Request Authenticator stays protocol-computed (accounting-style). |
| `sendDisconnect` | ✅ Retries up to `retry.maxAttempts` on: `timeout`, `malformed_response`, `identifier_mismatch`, `authenticator_mismatch`, `unknown_code` | `validateResponseSource`, `responseLengthValidationPolicy`, `responseMessageAuthenticatorPolicy`, `dynamicAuthorizationRetryIdentityMode`, `dynamicAuthorizationEventTimestampWindowSeconds` | `disconnect_nak` is terminal (no retry). Unexpected dynamic response codes are silently discarded (the call keeps waiting for Disconnect-ACK/NAK until timeout). Same host-aware stable identifier + source-port semantics as CoA. |
| health probes (`failover` / background checks) | 🚫 `retry.*` is not used for probe calls (single probe attempt per host per cycle) | `healthCheckProbeMode`, `healthCheckTimeoutMs`; auth/status-server probe path applies `validateResponseSource`, `authMethod`, `chapId`, `chapChallenge` | `healthCheckProbeMode: "status-server"` first uses `radiusStatusServerProbe`; non-healthy results/errors fall back to auth probes for compatibility. Accounting/CoA/Disconnect timeout probe paths currently do not forward `validateResponseSource` (they use protocol default strict source validation, effectively `true`). Accounting timeout probes explicitly set `responseLengthValidationPolicy: "strict"`; CoA/Disconnect timeout probe paths do not forward this option and rely on protocol defaults (currently `allow_trailing_bytes`). |
...
| Accounting retry identity semantics | `sendAccounting` | Retries to the same destination reuse one Accounting Identifier + source port per logical send. Failover-host retry uses a new identity, rotating both identifier and source port. |
| Dynamic authorization retry identity | `sendCoa`, `sendDisconnect`, `dynamicAuthorizationRetryIdentityMode` | `stable` (default) reuses one identifier + source port for retries to the same host and rotates both on failover-host retry; `per_attempt` uses a fresh identity per attempt. Request Authenticator remains protocol-computed per packet. |
...
- Health-probe length-policy caveat: accounting timeout probes explicitly pass `responseLengthValidationPolicy: "strict"`; CoA/disconnect timeout probe paths do not forward this option and therefore use protocol defaults (currently `allow_trailing_bytes`).
...
- Dynamic authorization responses (`sendCoa` / `sendDisconnect`) always reject invalid *present* Message-Authenticator values regardless of `responseMessageAuthenticatorPolicy`; missing Message-Authenticator remains acceptable.
- Dynamic authorization responses silently discard packets with unexpected response codes and keep waiting for ACK/NAK until timeout.
- `dynamicAuthorizationEventTimestampWindowSeconds` (default `300`) controls Event-Timestamp freshness validation for dynamic authorization responses.
